...
#    ground_truth = np.zeros( (256,512), np.uint16 );
...
def resize_dataset( parameters_data, subdir, generate_gt = True ):
    ...
    path_to_gt = os.path.join( path_to_dataset, 'gtFine' )
    ...
    text_file_name = 'cityscapes_' + subdir + '.txt'
    ...
    for city in all_cities:
        ...
        for file_name in all_files:
            ...
            path_to_resized_gt = path_to_resized_image
...
if __name__ == '__main__':
    ...
    resize_dataset(parameters_data, 'val')
    resize_dataset(parameters_data, 'train')
    resize_dataset(parameters_data, 'test', generate_gt = False)
